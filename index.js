      ...
      translations.forEach((trans, i) => {
        ...
        for (let word of words) {
          if (targetEntry.lines.length === amtOfLines
            ...
            debug('Entry', targetEntry.lineNumber, 'full, go to next one'); // TODO remove
            targetEntry = targetEntries.find(e => e.ts > targetEntry.ts);
            if (targetEntry === undefined) {
              // We're at the last one - just keep appending to it...
              debug('Entries full, appending to last one...'); // TODO remove
              overflow = true;
              targetEntry = targetEntries.find(e => e.ts >= ts);
            } else {
              debug('Entry', targetEntry.lineNumber, 'selected'); // TODO remove
            }
          }

          if (targetEntry.lines.length === 0) {
            // First word in this entry
            debug('Adding first word', word, 'to', targetEntry.lineNumber); // TODO remove
            ...
            debug('Adding first word', word, 'to new line of', targetEntry.lineNumber); // TODO remove
            targetEntry.lines.push(word);
          } else {
            // No problem to add this word to the last line in this entry
            debug('Adding word', word, 'to', targetEntry.lineNumber); // TODO remove
